...
import com.manisha.toursandtravels.dao.BookingDao;
import com.manisha.toursandtravels.pojo.Booking;
import com.manisha.toursandtravels.pojo.Trips;
...
public class BookingDaoImpl implements BookingDao
{
	...
	{
		sqlQuery="insert into booking_408 (custEmailId,tripId,bookingDate,noOfPersons,totalAmount) values(?,?,?,?,?)";
		try {
			...
			if(i>0)
			{
				sqlQuery2 = "select * from booking_408 where custEmailId=? and bookingDate=?";
	...
	public Booking updateBooking(int bookingId, int tripId, int noOfPerson) {
		...
		sqlQuery="update booking_408 set tripId=? , noOfPersons=? , totalAmount=?  where bookingId=?";
		try {
			ps = con.prepareStatement(sqlQuery);
			ps.setInt(1,tripId);
			...
			if(i>0)
			{
				sqlQuery2="select * from booking_408 where bookingId=?";
				ps2 = con.prepareStatement(sqlQuery2);
	...
	public boolean cancelBooking(int bookingId) {
		...
		sqlQuery ="delete from booking_408 where bookingId=?";
		try {
			ps = con.prepareStatement(sqlQuery);
	...
	public List<Booking> getAllBookings() {
		...
		sqlQuery="select * from booking_408";
		try {
			ps = con.prepareStatement(sqlQuery);
	...
	public List<Booking> getMyBookings(String custEmailId) {
		...
		List<Booking>  bookinglist = new ArrayList<>();
		sqlQuery="select * from booking_408 where custEmailId=?";
		try {
			ps = con.prepareStatement(sqlQuery);
			ps.setString(1,custEmailId);
