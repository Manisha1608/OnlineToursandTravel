...
import com.manisha.toursandtravels.dao.PackageDao;
import com.manisha.toursandtravels.pojo.Package;
import com.manisha.toursandtravels.utility.DBConnection;
import com.manisha.toursandtravels.utility.SortPackageByName;
...
public class PackageDaoImpl implements PackageDao 
{
	...
	{
		sqlQuery = "insert into package_408(packageName, packageLocation, packageType, packageDays, packagePrice, packageDetails, packageImage)"
				+ " values (?,?,?,?,?,?,?)";
		try 
		{
	...
	public List<Package> showAllPackage() 
	{
		...
		sqlQuery = "select * from package_408";
		try 
		{
			ps = con.prepareStatement(sqlQuery);
	...
	public boolean updatePackage(Package pkg) {
		...
		sqlQuery = "update package_408 set packageName=?, packageLocation=?, packageType=?, packageDays=?, packagePrice=?, packageDetails=?, packageImage=? where  packageId=?";
		try 
		{
			ps = con.prepareStatement(sqlQuery);
			ps.setString(1, pkg.getPackageName());
	...
	public boolean deletePackage(int packageId) {
		sqlQuery="delete from package_408 where packageId=?";
		try
		{
			ps = con.prepareStatement(sqlQuery);
	...
	public Package getPackageById(int packageId) {
		sqlQuery = "select * from package_408 where packageId=?";
		try 
		{
			ps = con.prepareStatement(sqlQuery);
			ps.setInt(1,packageId);
	...
	public List<Package> getPackageByLocation(String packageLocation) {
		...
		sqlQuery = "select *from package_408 where packagelocation =?";
		try {
			ps = con.prepareStatement(sqlQuery);
